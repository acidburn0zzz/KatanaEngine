...
void LeafFlow (int leafnum)
{
	leaf_t		*leaf;
	byte		*outbuffer;
	byte		compressed[BSP_MAX_LEAFS/8];
	int			i, j;
	int			numvis;
	byte		*dest;
	portal_t	*p;
...
//					Error ("No seperating planes found in portal pair");
					...
					sep = (sep_t*)qmalloc(sizeof(*sep));
					sep->next	= NULL;
					sep->plane	= p1->plane;
				}

				passages = (passage_t*)qmalloc(sizeof(*passages));
				passages->planes = sep;
...
"-farplane       limit visible distance (warning: not a good idea without fog)\n"
	...
	uncompressed = (byte*)qmalloc(bitlongs*portalleafs*sizeof(long));
