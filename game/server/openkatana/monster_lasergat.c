...
void LaserGat_HandleAim(edict_t *eLaserGat)
{
	...
	case THINK_IDLE:
		if(!eLaserGat->monster.eTarget)
		{
			...
			{
				eLaserGat->monster.iCommandList[COMMAND_LOOK_WAIT]--;
				break;
			}
	...
	if(eLaserGat->v.angles[1] > 360.0f)
		eLaserGat->v.angles[1] = 360.0f;
	// [17/2/2014] Oops, fixed a mistake here ~hogsy
	else if(eLaserGat->v.angles[1] < -360.0f)
		eLaserGat->v.angles[1] = -360.0f;
}
...
void LaserGat_Explode(edict_t *eLaserGat)
{
	...
	Entity_RadiusDamage(eLaserGat,MONSTER_RANGE_NEAR,25);
...
void LaserGat_Die(edict_t *eLaserGat,edict_t *eOther)
{
	...
	eLaserGat->v.dNextThink	= Server.dTime+10.0;
}
...
void LaserGat_Think(edict_t *eLaserGat)
{
	...
	case THINK_ATTACKING:
		{
			...
			//wLaserWeapon->Primary(eLaserGat);
		}
		break;
	default:
		Monster_SetThink(eLaserGat,THINK_IDLE);
	...
	// [15/3/2013] Always handle our aim! ~hogsy
...
void LaserGat_Spawn(edict_t *eLaserGat)
{
	...
	wLaserWeapon = Weapon_GetWeapon(WEAPON_LASERS);
