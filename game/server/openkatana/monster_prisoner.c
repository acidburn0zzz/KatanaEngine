...
void Prisoner_Stand(edict_t *ePrisoner)
{
#if 0
	BOOL bIsFree;
	...
	if(ePrisoner->v.iHealth <= 0)
		return;
	else if(ePrisoner->monster.iEmotion[EMOTION_BOREDOM] >= 5)
	{
		Monster_SetThink(ePrisoner,THINK_WANDERING);
		return;
	}
	else if(ePrisoner->monster.iEmotion[EMOTION_FEAR] >= 5)
	{
		Monster_SetThink(ePrisoner,THINK_FLEEING);
		return;
	}

	bIsFree = Prisoner_IsInClosedCell(ePrisoner);
	if(!bIsFree)
		if(rand()%19 == 1)
			Sound(ePrisoner,CHAN_VOICE,PRISONER_SOUND_HELP,255,ATTN_NORM);

	if(!ePrisoner->v.enemy || !Monster_IsVisible(ePrisoner,ePrisoner->v.enemy))
		Monster_SetEnemy(ePrisoner);
	else
	{
		if(!bIsFree && (ePrisoner->v.enemy->v.flags && FL_CLIENT))
		{
			// [21/10/2012] TODO: Make us watch the player ~hogsy
		}
		else if(bIsFree)
		{
			if(ePrisoner->v.enemy->monster.iRelationship					==
				ePrisoner->monster.iRelationship							&&
				MONSTER_GetRange(ePrisoner,ePrisoner->v.enemy->v.origin)	<
				MONSTER_RANGE_NEAR)
			{
				// [21/10/2012] TODO: Turn to face ~hogsy

				if(rand()%19 == 1 && bIsFree)
					Sound(ePrisoner,CHAN_VOICE,PRISONER_SOUND_HELP,255,ATTN_NORM);
			}
			else if(ePrisoner->v.enemy->monster.iRelationship != ePrisoner->monster.iRelationship)
			{
				Monster_SetThink(ePrisoner,THINK_FLEEING);
				return;
			}
	...
	ePrisoner->monster.eTarget = Monster_GetTarget(ePrisoner);
	if(ePrisoner->monster.eTarget)
	{
		if(Monster_SetEnemy(ePrisoner))
		{
			Monster_SetThink(ePrisoner,THINK_FLEEING);
			return;
		}
...
//	ePrisoner->monster.think_stand		= Prisoner_Stand;
