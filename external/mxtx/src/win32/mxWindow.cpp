...
int mxWindow::handleEvent (mxEvent *event)
{
	return 0;
}
...
void mxWindow::setTimer (int milliSeconds)
{
	if (d_this->d_uTimer)
	{
...
void mxWindow::setMenuBar (mxMenuBar *menuBar)
{
	SetMenu ((HWND) getHandle (), (HMENU) ((mxWidget *) menuBar)->getHandle ());
}

int mxWindow::doModal ()
{
	d_this->d_dlgRet = 0;
...
void mxWindow::endDialog (int retValue)
{
