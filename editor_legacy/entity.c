...
entity_t *Entity_Parse(bool onlypairs)
{
	...
	e = Eclass_ForName(ValueForKey(ent,"classname"),bHasBrushes);
	ent->eclass = e;
	if (e->fixedsize)
	{	// fixed size entity
		if (ent->brushes.onext != &ent->brushes)
		...
		Math_VectorAdd(e->mins,ent->origin, mins);
		Math_VectorAdd(e->maxs,ent->origin, maxs);
...
void Entity_Write (entity_t *e, FILE *f, bool use_region)
{
	...
	if (use_region)
	{
		...
		if ( !strcmp(ValueForKey (e, "classname"), "point_start") )
		{
			fprintf(f, "{\n");
			fprintf(f, "\"classname\" \"point_start\"\n");
	...
	{
		Math_VectorSubtract(e->brushes.onext->mins,e->eclass->mins,origin);
...
entity_t	*Entity_Create (eclass_t *c)
{
	...
	if (c->fixedsize)
	{
		...
		// create a custom brush
		Math_VectorAdd (c->mins, e->origin, mins);
		Math_VectorAdd (c->maxs, e->origin, maxs);
