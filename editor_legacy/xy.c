...
void XY_MouseDown(int x,int y,int buttons)
{
	...
	press_selection = (BOOL)(selected_brushes.next != &selected_brushes);
	...
	{	
		...
		if (point[1] || point[0])
		{
			camera.angles[YAW] = 180/pMath_PI*atan2 (point[1], point[0]);
			Sys_UpdateWindows (W_CAMERA|W_XY_OVERLAY);
		}
	}
...
void DrawCameraIcon (void)
{
	...
	x = camera.origin[0];
	y = camera.origin[1];
	a = camera.angles[YAW]/180.0f*pMath_PI;
	...
	glBegin(GL_LINE_STRIP);
	glVertex3f (x+48.0f*cos(a+pMath_PI/4), y+48*sin(a+pMath_PI/4), 0);
	glVertex3f (x, y, 0);
	glVertex3f (x+48*cos(a-pMath_PI/4), y+48*sin(a-pMath_PI/4), 0);
	glEnd ();
