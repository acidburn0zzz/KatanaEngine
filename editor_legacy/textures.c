...
qtexture_t *Texture_CreateSolid (char *name)
{
	...
	data[0] = (byte)q->color[0]*255;
	data[1] = (byte)q->color[1]*255;
	data[2] = (byte)q->color[2]*255;
	data[3] = 255;
	...
	for(;;)
	{
			...
			&& (y > g_qeglobals.d_texturewin.originy - height) )
		{
			...
			if (!strcmpi(g_qeglobals.d_texturewin.texdef.name, q->name))
			{
				...
				glDisable(GL_TEXTURE_2D);
				glBegin(GL_LINE_LOOP);
				glVertex2f(x-2,y-FONT_HEIGHT+2);
				glVertex2f(x-2,y-FONT_HEIGHT-q->height-2);
				glVertex2f(x+2+q->width,y-FONT_HEIGHT-q->height-2);
				glVertex2f(x+2+q->width,y-FONT_HEIGHT+2);
				glEnd();
				glEnable(GL_TEXTURE_2D);
				glLineWidth(1.0f);
			}
