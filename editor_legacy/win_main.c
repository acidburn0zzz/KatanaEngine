...
//
		...
		case ID_FILE_NEW:
			...
			break;
		case ID_FILE_SAVE:
			if(!strcmp(currentmap,"unnamed"LVL_EXTENSION))
				SaveAsDialog();
			else
...
void Main_Create (HINSTANCE hInstance)
{
	...
	// Create child window.
	{
		...
		cStruct.hWindowMenu		= GetSubMenu(GetMenu(g_qeglobals.d_hwndMain),2);
		cStruct.idFirstChild	= ID_MAINCHILD;

		g_qeglobals.hChildParent = CreateWindowEx(
			WS_EX_CLIENTEDGE,
			"childparent",
			NULL,
...
HWND CreateToolBar(HINSTANCE hinst)
{
	...
	// [8/8/2012] Set TB_SETMAXTEXTROWS ~hogsy
	SendMessage(g_qeglobals.hToolBar,TB_SETMAXTEXTROWS,(WPARAM)0,0);
