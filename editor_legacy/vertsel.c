...
int	FindPoint (vec3_t point)
{
	...
	Math_VectorCopy(point,g_qeglobals.d_points[g_qeglobals.d_numpoints]);
...
void SelectFaceEdge (face_t *f, int p1, int p2)
{
	...
	for (i=0 ; i<w->numpoints ; i++)
		...
		{
			Math_VectorCopy(g_qeglobals.d_points[pnum[i]],f->planepts[0]);
			Math_VectorCopy(g_qeglobals.d_points[pnum[(i+1)%w->numpoints]],f->planepts[1]);
			Math_VectorCopy(g_qeglobals.d_points[pnum[(i+2)%w->numpoints]],f->planepts[2]);
			for (j=0 ; j<3 ; j++)
				for (k=0 ; k<3 ; k++)
...
void SelectVertex (int p1)
{
	...
	for (f=b->brush_faces ; f ; f=f->next)
	{
		...
		for (i=0 ; i<w->numpoints ; i++)
		{
			...
			{
				Math_VectorCopy(w->points[(i+w->numpoints-1)%w->numpoints],f->planepts[0]);
				Math_VectorCopy(w->points[i],f->planepts[1]);
				Math_VectorCopy(w->points[(i+1)%w->numpoints],f->planepts[2]);
...
void SelectEdgeByRay (vec3_t org, vec3_t dir)
{
	...
	for (i=0 ; i<g_qeglobals.d_numedges ; i++)
	{
		...
		Math_VectorSubtract (mid, org, temp);
		d = Math_DotProduct (temp, dir);
		Math_VectorMA (org, d, dir, temp);
		Math_VectorSubtract (mid, temp, temp);
		d = Math_VectorLength (temp);
		if (d < bestd)
		{
			bestd = d;
			besti = i;
...
void SelectVertexByRay (vec3_t org, vec3_t dir)
{
	...
	for (i=0 ; i<g_qeglobals.d_numpoints ; i++)
	{
		Math_VectorSubtract (g_qeglobals.d_points[i], org, temp);
		d = Math_DotProduct (temp, dir);
		Math_VectorMA (org, d, dir, temp);
		Math_VectorSubtract (g_qeglobals.d_points[i], temp, temp);
		d = Math_VectorLength (temp);
		if (d < bestd)
		{
			bestd = d;
