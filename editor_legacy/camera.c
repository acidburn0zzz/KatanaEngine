...
{
	float	xa,ya,matrix[4][4];
	int		i;

	xa = camera.angles[0]/180.0f*(float)pMath_PI;
	ya = camera.angles[1]/180.0f*(float)pMath_PI;
	...
	Math_VectorNormalize(camera.vright);
	Math_VectorNormalize(camera.vup);
	Math_VectorNormalize(camera.vpn);
}
...
void Cam_MouseControl(float dtime)
{
	...
	if(iCameraButtonState != MK_RBUTTON)
		return;
	...
	camera.angles[YAW]		+= xf*-dtime*300.0f;
	camera.angles[PITCH]	+= yf*dtime*400.0f;

	// [22/8/2012] Don't let us look too low or too high ~hogsy
	if(camera.angles[PITCH] > 90.0f)
		camera.angles[PITCH] = 90.0f;
	else if(camera.angles[PITCH] < -90.0f)
		camera.angles[PITCH] = -90.0f;

	GetWindowRect(g_qeglobals.d_hwndCamera,&g_qeglobals.rRectCamera);

	SetCursorPos(
		(g_qeglobals.rRectCamera.left+g_qeglobals.rRectCamera.right)/2,
		(g_qeglobals.rRectCamera.top+g_qeglobals.rRectCamera.bottom)/2);
...
void Camera_Draw(void)
{
	...
	yfov = 2*atanf((float)camera.height/camera.width)*180.0f/pMath_PI;
