...
static char szFilter[260] =     /* filter string                 */
	"Map file (*"EXTENSION_MAP")\0*"EXTENSION_MAP"\0\0";
...
void ProjectDialog (void)
{
	...
	{
		gWindow_MessageBox("Katana Editor","Failed to load project file! (%s)",ofn.lpstrFile);
...
void SaveAsDialog (void)
{
	...
	DefaultExtension(ofn.lpstrFile,EXTENSION_MAP);
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow)
{
	...
	if(lpCmdLine && strlen(lpCmdLine))
	{
		...
		{
			gWindow_MessageBox("Katana Editor","Failed to load project file! (%s)",argv[1]);
	...
	{
		gWindow_MessageBox("Katana Editor","Failed to load project file! (%s)",PATH_ENGINE"/scripts/paths.script");
