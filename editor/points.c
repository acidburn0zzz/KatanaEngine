...
void Pointfile_Next (void)
{
	...
	Math_VectorCopy(s_pointvecs[s_check_point],camera.origin);
	Math_VectorCopy(s_pointvecs[s_check_point],g_qeglobals.d_xy.origin);
	Math_VectorSubtract(s_pointvecs[s_check_point+1], camera.origin, dir);
	Math_VectorNormalize(dir);

	camera.angles[1] = (float)atan2(dir[1],dir[0])*180.0f/3.14159f;
	camera.angles[0] = (float)asin(dir[2])*180.0f/3.14159f;
...
void Pointfile_Check (void)
{
	...
	do
	{
		...
		{
			Math_VectorCopy(v,s_pointvecs[s_num_points]);
			s_num_points++;
		}
		glVertex3fv (v);
	} while (1);
