...
void Select_Clone (void)
{
	...
	delta[0] = (float)g_qeglobals.d_gridsize;
	delta[1] = (float)g_qeglobals.d_gridsize;
	delta[2] = 0;
...
void Select_GetMid (vec3_t mid)
{
	...
	for(i = 0; i < 3; i++)
		mid[i] = (float)g_qeglobals.d_gridsize*floor(((mins[i]+maxs[i])*0.5)/g_qeglobals.d_gridsize);
}
