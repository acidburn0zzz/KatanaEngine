...
void Map_Free (void)
{
	...
	Texture_ClearInuse ();
	Pointfile_Clear ();
	strcpy(currentmap,"unnamed"EXTENSION_MAP);
...
void Map_LoadFile (char *filename)
{
	...
	Console_Print(false,"--- LoadMapFile ---\n");
	Console_Print(false,"%s\n", temp );

	Console_Print(false,"%5i brushes\n",  g_qeglobals.d_parsed_brushes );
	Console_Print(false,"%5i entities\n", g_qeglobals.d_num_entities);

	Map_RestoreBetween ();

	Console_Print(false,"Map_BuildAllDisplayLists\n");
	...
	else
	{
		...
		Math_VectorCopy(vec3_origin,camera.origin);
		Math_VectorCopy(vec3_origin,g_qeglobals.d_xy.origin);
	}
...
void Map_SaveFile (char *filename,bool use_region )
{
	...
	if(!f)
	{
		Console_Print(false,"ERROR!!!! Couldn't open %s\n",filename);
		return;
	}
...
void Map_RegionOff (void)
{
	...
	for (i=0 ; i<3 ; i++)
	{
		region_maxs[i] = 4096.0f;
		region_mins[i] = -4096.0f;
	}
...
void Map_RegionXY (void)
{
	...
	region_mins[0] = g_qeglobals.d_xy.origin[0]-0.5f*g_qeglobals.d_xy.width/g_qeglobals.d_xy.scale;
	region_maxs[0] = g_qeglobals.d_xy.origin[0]+0.5f*g_qeglobals.d_xy.width/g_qeglobals.d_xy.scale;
	region_mins[1] = g_qeglobals.d_xy.origin[1]-0.5f*g_qeglobals.d_xy.height/g_qeglobals.d_xy.scale;
	region_maxs[1] = g_qeglobals.d_xy.origin[1]+0.5f*g_qeglobals.d_xy.height/g_qeglobals.d_xy.scale;
	region_mins[2] = -4096;
	region_maxs[2] = 4096;
...
void Map_RegionTallBrush (void)
{
	...
	Math_VectorCopy(b->mins,region_mins);
	Math_VectorCopy(b->maxs,region_maxs);
