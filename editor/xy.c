...
void XY_ToGridPoint (int x, int y, vec3_t point)
{
	...
	point[1] = g_qeglobals.d_xy.origin[1] + (y - g_qeglobals.d_xy.height/2)/g_qeglobals.d_xy.scale;
	point[2] = 0;
	point[0] = floor(point[0]/g_qeglobals.d_gridsize+0.5f)*g_qeglobals.d_gridsize;
	point[1] = floor(point[1]/g_qeglobals.d_gridsize+0.5f)*g_qeglobals.d_gridsize;
}

void XY_MouseDown(int x,int y,int buttons)
{
	vec3_t	point,origin,dir,right,up;
	...
	Math_VectorCopy (point, origin);
	origin[2] = 8192.0f;

	Math_VectorClear(dir);
	Math_VectorClear(right);
	Math_VectorClear(up);

	dir[2]		= -1.0f;
	right[0]	= 1.0f/g_qeglobals.d_xy.scale;
	up[1]		= 1.0f/g_qeglobals.d_xy.scale;

	bPressSelection = (bool)(selected_brushes.next != &selected_brushes);
	...
	else if(buttonstate == (MK_CONTROL|MK_MBUTTON))
	{
	...
	{
		Math_VectorSubtract (point, camera.origin, point);
		if (point[1] || point[0])
		{
...
BOOL DragDelta(int x,int y,vec3_t move)
{
	...
	Math_VectorSubtract(delta,pressdelta,move);
	Math_VectorCopy(delta,pressdelta);
...
void XY_MouseMoved(int x,int y,int buttons)
{
	...
	if(buttonstate == MK_LBUTTON && !bPressSelection)
	{
		NewBrushDrag(x,y);
		return;
	}
	// lbutton with selection = drag selection
	else if(buttonstate & MK_LBUTTON)
	{
...
void XY_DrawGrid (void)
{
	...
	// draw minor blocks
	if(g_qeglobals.bShowGrid && g_qeglobals.d_gridsize*g_qeglobals.d_xy.scale >= 4)
	{
...
void DrawCameraIcon (void)
{
	...
	x = camera.origin[0];
	y = camera.origin[1];
	a = camera.angles[YAW]/180.0f*M_PI;
	...
	glVertex3f(x-16.0f,y,0);
	glVertex3f(x,y+8.0f,0);
	glVertex3f(x+16.0f,y,0);
	glVertex3f(x,y-8.0f,0);
	glVertex3f(x-16.0f,y,0);
	glVertex3f(x+16.0f,y,0);
	...
	glBegin(GL_LINE_STRIP);
	glVertex3f (x+48.0f*cos(a+M_PI/4), y+48*sin(a+M_PI/4), 0);
	glVertex3f (x, y, 0);
...
BOOL FilterBrush(brush_t *pb)
{
	...
	{
		if (!strncmp(pb->owner->eclass->name,ENTITY_LIGHT, 5))
			return TRUE;
	}
...
void DrawPathLines (void)
{
	...
	for (se = entities.next ; se != &entities ; se = se->next)
	{
		...
		for (k=0 ; k<num_entities ; k++)
		{
			...
			Math_VectorSubtract (mid1, mid, dir);
			len = Math_VectorNormalize (dir);
...
void XY_Draw (void)
{
	...
	for (brush = active_brushes.next ; brush != &active_brushes ; brush=brush->next)
	{
		...
		// [11/8/2012] TODO: This is a tad confusing... ~hogsy
		if (brush->owner != e)
			e = brush->owner;

		Brush_DrawXY(brush,e->eclass->color);
	}
...
void XY_Overlay (void)
{
	...
	//
	Math_VectorCopy(z.origin,lastz);
	Math_VectorCopy(camera.origin,lastcamera);
