...
bool QE_LoadProject(char *cProjectFile)
{
	...
	if(LoadFile(cProjectFile,(void**)&data) == -1)
		return false;
	...
	{
		gWindow_MessageBox("Katana Editor","Failed to parse project file! (%s)",cProjectFile);
	...
	Eclass_InitForSourceDirectory(ValueForKey(g_qeglobals.d_project_entity,SCRIPT_FLAG_ENTITYPATH));
...
EngineImport_t  *eiEditor;

gINSTANCE   iEngineInstance;
