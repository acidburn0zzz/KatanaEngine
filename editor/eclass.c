...
void Eclass_ScanFile (char *filename)
{
	...
	Console_Print(FALSE,"ScanFile: %s\n", temp);
...
void Eclass_InitForSourceDirectory (char *path)
{
	...
	int		handle;
	char	filename[1024],filebase[1024],temp[1024];
	char	*s;

	QE_ConvertDOSToUnixName( temp, path );

	Console_Print(FALSE,"Eclass_InitForSourceDirectory: %s\n", temp );
...
eclass_t *Eclass_ForName(char *name,BOOL bHasBrushes)
{
	eclass_t	*e;
	char		init[1024];
