...
// rotate axis
	...
	{
		ang = f->texdef.rotate / 180.0f*M_PI;
		sinv = sin(ang);
		cosv = cos(ang);
	}
...
void DrawBrushEntityName (brush_t *b)
{
	...
	if(!b->owner || (b->owner == world_entity) || (b != b->owner->brushes.onext))
		return;
...
void Brush_SnapPlanepts (brush_t *b)
{
	...
	for (f=b->brush_faces ; f; f=f->next)
		for (i=0 ; i<3 ; i++)
			for (j=0 ; j<3 ; j++)
				f->planepts[i][j] = floorf(f->planepts[i][j]+0.5f);
}
...
brush_t *Brush_Parse(void)
{
	...
	// read the texturedef
		...
		GetToken(false);
		f->texdef.scale[1] = (float)atof(token);
...
void Brush_MakeSided(int sides)
{
	...
	for (i=0 ; i<sides ; i++)
	{
		...
		sv = sinf(i*3.14159265*2/sides);
		cv = cosf(i*3.14159265*2/sides);
...
void Brush_DrawXY(brush_t *b,vec3_t vColor)
{
	face_t		*face;
	int			order;
	winding_t	*w;
...
//		glVertexPointer(3,GL_FLOAT,0,w->points);
//		glDrawArrays(GL_LINE_LOOP,1,w->numpoints);
